...
if __name__ == "__main__":
    ...
    X_train, y_train = SVHN_Full(train_dir).generate_data()
    X_test, y_test = SVHN_Full(test_dir).generate_data()
    X_extra, y_extra = SVHN_Full(extra_dir).generate_data()


    def train_validation_sets(X_train, y_train, X_extra, y_extra):
        import random
        valid_index = []
        valid_index2 = []
        train_index = []
        train_index2 = []
        for i in np.arange(10):
            valid_index.extend(np.where(y_train[:, 1] == (i))[0][:400].tolist())
            train_index.extend(np.where(y_train[:, 1] == (i))[0][400:].tolist())
            valid_index2.extend(np.where(y_extra[:, 1] == (i))[0][:200].tolist())
            train_index2.extend(np.where(y_extra[:, 1] == (i))[0][200:].tolist())
        ...
        valid_dataset = np.concatenate((X_extra[valid_index2, :, :, :], X_train[valid_index, :, :, :]),
                                       axis=0)
        valid_labels = np.concatenate((y_extra[valid_index2, :], y_train[valid_index, :]), axis=0)
        train_dataset_t = np.concatenate((X_extra[train_index2, :, :, :], X_train[train_index, :, :, :]),
                                         axis=0)
        train_labels_t = np.concatenate((y_extra[train_index2, :], y_train[train_index, :]), axis=0)
        return train_dataset_t, train_labels_t, valid_dataset, valid_labels

    X_train, y_train, X_valid, y_valid = train_validation_sets(X_train, y_train, X_extra, y_extra)

    from six.moves import cPickle as pickle

    f = open("../SVHN_case/SVHN_MultiDigit.pickle", 'wb')
    save = {
        'X_train': X_train,
        'y_train': y_train,
        'X_valid': X_valid,
